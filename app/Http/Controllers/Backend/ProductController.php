...
class ProductController extends Controller
{
    ...
    public function productStore (Request $request)
    {
        ...
        if(isset($request->image)){
            $imageName = rand().'-product-'.'.'.$request->image->extension(); // 85778-category-.jpg
        ...
        if(isset($request->galleryImage)){
            foreach($request->galleryImage as $image){
                ...
                $image->move('backend/images/gallerImage',$imageName);
    ...
    public function productDelete ($id)
    {
        ...
        if($galleryImages->isNotEmpty()){
            foreach($galleryImages as $image){
                ...
                if($image->image && file_exists('backend/images/gallerImage/'.$image->image)){
                    unlink('backend/images/gallerImage/'.$image->image);
                }
        ...
        if(isset($request->galleryImage)){
            ...
            if($images->isNotEmpty()){
                foreach($images as $singleImage){
                    if($singleImage->image && file_exists('backend/images/gallerImage/'.$singleImage->image)){
                        unlink('backend/images/gallerImage/'.$singleImage->image);
                    }
